...
import type { Era, GameModeId, GameState, RosterFormatId } from '../lib/types'
...
export default function GameRoute({ gameModeId = 'all-time' }: GameRouteProps) {
  const dataset = useMemo(() => getModeDataset(gameModeId), [gameModeId])
  const teamReel = useMemo(
    () =>
      [...new Set(dataset.draftBuckets.map((bucket) => bucket.teamName))].sort(
        (a, b) => a.localeCompare(b),
      ),
    [dataset.draftBuckets],
  )

  const [gameState, setGameState] = useState<GameState>(() =>
    createInitialGameState('classic', gameModeId),
  )
  ...
  const teamPreview = useMemo(() => {
    if (teamReel.length === 0) return ''
    return teamReel[teamSpinTick % teamReel.length] ?? ''
  }, [teamReel, teamSpinTick])
  ...
  const selectedPlayer = useMemo(() => {
    ...
    return dataset.playerById.get(gameState.selectedPlayerId) ?? null
  }, [dataset.playerById, gameState.selectedPlayerId])
  ...
  }, [gameState.status, seasonResult, simulationReroll, gameModeId])

  const handleStart = useCallback(
    (formatId: RosterFormatId) => {
      setGameState(startGame(createInitialGameState(formatId, gameModeId)))
    },
    [gameModeId],
  )
  ...
  const handleRestart = useCallback(() => {
    ...
    setGameState(restartGame(gameState.rosterFormatId, gameModeId))
  }, [gameModeId, gameState.rosterFormatId])
  ...
  if (gameState.status === 'intro') {
    return (
      <div className="min-h-0 flex-1 overflow-y-auto">
        <HowToPlay gameModeId={gameModeId} onStart={handleStart} />
      </div>
    )
  }
